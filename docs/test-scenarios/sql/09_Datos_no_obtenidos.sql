-- Escenario 09: datos desactualizados.
--
-- Objetivo:
-- - Cargar datos coherentes y en estado verde.
-- - Dejar las fechas de actualización antiguas para validar STALE/desactualizado.
-- - No representar una caída técnica, sino falta de datos recientes.
--
-- Si tu umbral STALE es mayor de 5 días, aumenta @stale_days.
-- Conviene que @stale_days sea menor que el umbral de APs inactivos si no quieres
-- que Aruba marque APs inactivos por antigüedad.

START TRANSACTION;

SET @stale_days := 5;
SET @stale_time := DATE_SUB(NOW(), INTERVAL @stale_days DAY);
...
SELECT
    numbers.day_index,
    CASE
        WHEN numbers.day_index = 89 THEN @stale_time
        ELSE DATE_ADD(
            DATE_SUB(DATE(@stale_time), INTERVAL (89 - numbers.day_index) DAY),
            INTERVAL 12 HOUR
        )
    END AS snapshot_time,

    -- Plataformas técnicamente correctas.
    3 + MOD(numbers.day_index, 3) AS aruba_affectation,
    120 + MOD(numbers.day_index, 8) AS aruba_wifi_clients,
    8 + MOD(numbers.day_index, 3) AS aruba_open_tickets,

    180 + MOD(numbers.day_index, 12) AS citrix_active_sessions,
    4 AS citrix_available_delivery_controllers,
    MOD(numbers.day_index, 3) AS citrix_failed_logons,
    10 + MOD(numbers.day_index, 4) AS citrix_open_tickets,
    3 + MOD(numbers.day_index, 3) AS citrix_affectation,

    1200 + MOD(numbers.day_index, 50) AS microsoft365_active_users,
    9 + MOD(numbers.day_index, 3) AS microsoft365_open_tickets,
    3 + MOD(numbers.day_index, 3) AS microsoft365_affectation,

    6 + MOD(numbers.day_index, 4) AS glpi_operational_pressure,
    5 + MOD(numbers.day_index, 3) AS technical_degradation,
    3 + MOD(numbers.day_index, 3) AS user_impact,
    5 + MOD(numbers.day_index, 3) AS global_criticality,
    0 AS affected_services_percent,
    0 AS sla_risk,

    22 + MOD(numbers.day_index, 5) AS created_today,
    22 + MOD(numbers.day_index, 5) AS closed_today,
    120 + MOD(numbers.day_index, 8) AS created_this_week,
    120 + MOD(numbers.day_index, 8) AS closed_this_week
...
-- Citrix: valores correctos, pero collected_at antiguo.
...
-- Microsoft 365: valores correctos, pero collected_at antiguo.
...
-- GLPI: carga normal, pero collected_at antiguo.
...
-- KPIs transversales: situación verde, pero con histórico antiguo.
...
SELECT
    ...
    CASE kpi.kpi_code
        ...
        WHEN 'operational_priority' THEN 5 + MOD(days.day_index, 3)
...
-- Snapshots para análisis: verdes, pero el último ya es antiguo.
...
-- Aruba operativo: datos coherentes, pero actualizados hace varios días.
INSERT INTO `access_points` (
    ...
    ('TFG-AP-01', 'Up', '10.10.10.11', '80.10.10.11', 'TFG-AP-0001', 'Sede TFG', '10.6.0.0', '00:11:22:33:44:01', 'TFG-SWARM', NOW() - INTERVAL 120 DAY, @stale_time),
    ('TFG-AP-02', 'Up', '10.10.10.12', '80.10.10.12', 'TFG-AP-0002', 'Sede TFG', '10.6.0.0', '00:11:22:33:44:02', 'TFG-SWARM', NOW() - INTERVAL 120 DAY, @stale_time),
    ('TFG-AP-03', 'Up', '10.10.10.13', '80.10.10.13', 'TFG-AP-0003', 'Sede TFG', '10.6.0.0', '00:11:22:33:44:03', 'TFG-SWARM', NOW() - INTERVAL 120 DAY, @stale_time),
    ('TFG-AP-04', 'Up', '10.10.10.14', '80.10.10.14', 'TFG-AP-0004', 'Sede TFG', '10.6.0.0', '00:11:22:33:44:04', 'TFG-SWARM', NOW() - INTERVAL 120 DAY, @stale_time),
    ('TFG-AP-05', 'Up', '10.10.10.15', '80.10.10.15', 'TFG-AP-0005', 'Sede TFG', '10.6.0.0', '00:11:22:33:44:05', 'TFG-SWARM', NOW() - INTERVAL 120 DAY, @stale_time),
    ('TFG-AP-06', 'Up', '10.10.10.16', '80.10.10.16', 'TFG-AP-0006', 'Sede TFG', '10.6.0.0', '00:11:22:33:44:06', 'TFG-SWARM', NOW() - INTERVAL 120 DAY, @stale_time);
...
INSERT INTO `aruba_switches` (
    ...
    ('TFG-SW-0001', '00:AA:BB:CC:DD:01', 'TFG-SW-01', 'Aruba 2930F', 'Up', FALSE, 'Up', NOW() - INTERVAL 120 DAY, @stale_time),
    ('TFG-SW-0002', '00:AA:BB:CC:DD:02', 'TFG-SW-02', 'Aruba 2930F', 'Up', FALSE, 'Up', NOW() - INTERVAL 120 DAY, @stale_time),
    ('TFG-SW-0003', '00:AA:BB:CC:DD:03', 'TFG-SW-03', 'Aruba 2930F', 'Up', FALSE, 'Up', NOW() - INTERVAL 120 DAY, @stale_time);
...
INSERT INTO `aruba_switch_client_usage` (
    ...
    ('TFG-SW-0001', 'TFG-SW-01', '00:AA:BB:CC:DD:01', 'Up', 2, @stale_time),
    ('TFG-SW-0002', 'TFG-SW-02', '00:AA:BB:CC:DD:02', 'Up', 1, @stale_time),
    ('TFG-SW-0003', 'TFG-SW-03', '00:AA:BB:CC:DD:03', 'Up', 0, @stale_time);
