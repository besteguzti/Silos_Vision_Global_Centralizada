-- Escenario 05: Microsoft 365 con problema de identidad / seguridad.
--
-- Objetivo:
-- - Simular 90 dias historicos.
-- - Mantener Aruba correcto.
-- - Mantener Citrix correcto.
-- - Degradar Microsoft 365 por indicadores de identidad y seguridad:
--   usuarios sin MFA, usuarios en riesgo, inicios fallidos, equipos no conformes
--   y dispositivos sin cifrado.
-- - Aumentar tickets GLPI relacionados con Microsoft 365.
-- - Dejar el ultimo dia como estado actual degradado de Microsoft 365.
--
-- Recomendacion:
-- Ejecutar antes:
...
-- Limpieza defensiva para poder ejecutar este escenario de forma autocontenida.
...
SELECT
    ...
    -- Microsoft 365: usuarios activos bajan parcialmente por problemas de identidad.
    CASE
        WHEN base.day_index <= 44 THEN 1200 + MOD(base.day_index, 50)
        WHEN base.day_index <= 64 THEN 1160 - ((base.day_index - 45) * 8) + MOD(base.day_index, 7)
        WHEN base.day_index <= 79 THEN 1000 - ((base.day_index - 65) * 10) + MOD(base.day_index, 9)
        ELSE 840 - ((base.day_index - 80) * 8) + MOD(base.day_index, 5)
    END AS microsoft365_active_users,

    -- Indicadores de identidad / seguridad.
    CASE
        WHEN base.day_index <= 44 THEN 0
        WHEN base.day_index <= 64 THEN 2 + MOD(base.day_index, 3)
        WHEN base.day_index <= 79 THEN 8 + MOD(base.day_index, 5)
        ELSE 14 + MOD(base.day_index, 6)
    END AS risky_users,

    CASE
        WHEN base.day_index <= 44 THEN 1 + MOD(base.day_index, 4)
        WHEN base.day_index <= 64 THEN 18 + ((base.day_index - 45) * 2) + MOD(base.day_index, 5)
        WHEN base.day_index <= 79 THEN 65 + ((base.day_index - 65) * 3) + MOD(base.day_index, 6)
        ELSE 120 + ((base.day_index - 80) * 4) + MOD(base.day_index, 8)
    END AS microsoft365_failed_sign_ins,

    CASE
        WHEN base.day_index <= 44 THEN 0
        WHEN base.day_index <= 64 THEN 4 + MOD(base.day_index, 3)
        WHEN base.day_index <= 79 THEN 10 + MOD(base.day_index, 5)
        ELSE 18 + MOD(base.day_index, 6)
    END AS users_without_mfa,

    CASE
        WHEN base.day_index <= 44 THEN 8 + MOD(base.day_index, 10)
        WHEN base.day_index <= 64 THEN 35 + (base.day_index - 45) + MOD(base.day_index, 6)
        WHEN base.day_index <= 79 THEN 70 + ((base.day_index - 65) * 2) + MOD(base.day_index, 7)
        ELSE 105 + ((base.day_index - 80) * 2) + MOD(base.day_index, 8)
    END AS non_compliant_devices,

    CASE
        WHEN base.day_index <= 44 THEN 0
        WHEN base.day_index <= 64 THEN 2 + MOD(base.day_index, 3)
        WHEN base.day_index <= 79 THEN 6 + MOD(base.day_index, 4)
        ELSE 12 + MOD(base.day_index, 5)
    END AS devices_without_encryption,

    -- Tickets Microsoft 365 suben claramente.
    CASE
        WHEN base.day_index <= 44 THEN 9 + MOD(base.day_index, 3)
        WHEN base.day_index <= 64 THEN 18 + ((base.day_index - 45) * 2) + MOD(base.day_index, 4)
        WHEN base.day_index <= 79 THEN 55 + ((base.day_index - 65) * 2) + MOD(base.day_index, 5)
        ELSE 85 + (base.day_index - 80) + MOD(base.day_index, 4)
    END AS microsoft365_open_tickets,

    -- Afeccion Microsoft 365 normalizada.
    CASE
        WHEN base.day_index <= 44 THEN 3 + MOD(base.day_index, 4)
        WHEN base.day_index <= 64 THEN 28 + (base.day_index - 45)
        WHEN base.day_index <= 79 THEN 52 + (base.day_index - 65)
        ELSE 70 + (base.day_index - 80)
    END AS microsoft365_affectation,

    -- Presion operativa GLPI provocada por Microsoft 365.
    CASE
        WHEN base.day_index <= 44 THEN 6 + MOD(base.day_index, 4)
        WHEN base.day_index <= 64 THEN 24 + (base.day_index - 45)
        WHEN base.day_index <= 79 THEN 48 + (base.day_index - 65)
        ELSE 66 + (base.day_index - 80)
    END AS glpi_operational_pressure,

    -- Degradacion tecnica agregada.
    CASE
        WHEN base.day_index <= 44 THEN 5 + MOD(base.day_index, 3)
        WHEN base.day_index <= 64 THEN 26 + (base.day_index - 45)
        WHEN base.day_index <= 79 THEN 50 + (base.day_index - 65)
        ELSE 68 + (base.day_index - 80)
    END AS technical_degradation,

    -- Impacto en usuarios por problemas de acceso/identidad.
    CASE
        WHEN base.day_index <= 44 THEN 3 + MOD(base.day_index, 3)
        WHEN base.day_index <= 64 THEN 22 + (base.day_index - 45)
        WHEN base.day_index <= 79 THEN 46 + (base.day_index - 65)
        ELSE 64 + (base.day_index - 80)
    END AS user_impact,
    ...
    CASE
        WHEN base.day_index <= 44 THEN 5 + MOD(base.day_index, 3)
        WHEN base.day_index <= 64 THEN 22 + (base.day_index - 45)
        WHEN base.day_index <= 79 THEN 42 + (base.day_index - 65)
        ELSE 58 + (base.day_index - 80)
    END AS global_criticality,

    CASE
        WHEN base.day_index <= 44 THEN 0
        WHEN base.day_index <= 64 THEN 15
        WHEN base.day_index <= 79 THEN 30
        ELSE 40
    END AS affected_services_percent,

    CASE
        WHEN base.day_index <= 44 THEN 0
        WHEN base.day_index <= 64 THEN 8 + MOD(base.day_index, 4)
        WHEN base.day_index <= 79 THEN 16 + MOD(base.day_index, 5)
        ELSE 24 + MOD(base.day_index, 6)
    END AS sla_risk
...
-- Historico Microsoft 365: degradado por identidad/seguridad.
...
SELECT
    ...
    0,
    CASE
        WHEN day_index <= 44 THEN 0
        WHEN day_index <= 64 THEN 4 + MOD(day_index, 3)
        WHEN day_index <= 79 THEN 12 + MOD(day_index, 5)
        ELSE 22 + MOD(day_index, 6)
    END,
    45 + MOD(day_index, 15),
    risky_users,
    microsoft365_failed_sign_ins,
    users_without_mfa,
    CASE
        WHEN day_index <= 64 THEN 0
        ELSE 2 + MOD(day_index, 3)
    END,
    0,
    CASE
        WHEN day_index <= 64 THEN 0
        ELSE 3 + MOD(day_index, 4)
    END,
    non_compliant_devices,
    CASE
        WHEN day_index <= 64 THEN 0
        ELSE 8 + MOD(day_index, 6)
    END,
    devices_without_encryption,
    CASE
        WHEN day_index <= 64 THEN 0
        ELSE 15 + MOD(day_index, 6)
    END,
    CASE
        WHEN microsoft365_affectation >= 67 THEN 'RED'
        WHEN microsoft365_affectation >= 34 THEN 'YELLOW'
        ELSE 'GREEN'
    END,
    snapshot_time
FROM `test_scenario_days`;

-- Historico GLPI: presion centrada en tickets Microsoft 365.
...
SELECT
    ...
    CASE
        WHEN day_index <= 44 THEN 0
        WHEN day_index <= 64 THEN 2 + MOD(day_index, 3)
        WHEN day_index <= 79 THEN 5 + MOD(day_index, 4)
        ELSE 9 + MOD(day_index, 4)
    END,
    CASE
        WHEN day_index <= 44 THEN 0
        WHEN day_index <= 64 THEN 4 + MOD(day_index, 3)
        WHEN day_index <= 79 THEN 8 + MOD(day_index, 4)
        ELSE 13 + MOD(day_index, 5)
    END,
    CASE
        WHEN day_index <= 44 THEN 6
        WHEN day_index <= 64 THEN 11
        WHEN day_index <= 79 THEN 16
        ELSE 21
    END,
    CASE
        WHEN day_index <= 44 THEN 20 + MOD(day_index, 4)
        WHEN day_index <= 64 THEN 34 + MOD(day_index, 6)
        WHEN day_index <= 79 THEN 46 + MOD(day_index, 8)
        ELSE 56 + MOD(day_index, 8)
    END,
    CASE
        WHEN day_index <= 44 THEN 20 + MOD(day_index, 4)
        WHEN day_index <= 64 THEN 18 + MOD(day_index, 4)
        WHEN day_index <= 79 THEN 16 + MOD(day_index, 4)
        ELSE 14 + MOD(day_index, 4)
    END,
    120 + MOD(day_index, 6) + aruba_open_tickets + citrix_open_tickets + microsoft365_open_tickets,
    CASE
        WHEN day_index <= 44 THEN 120 + MOD(day_index, 6)
        WHEN day_index <= 64 THEN 100 + MOD(day_index, 6)
        WHEN day_index <= 79 THEN 85 + MOD(day_index, 6)
        ELSE 70 + MOD(day_index, 6)
    END,
    aruba_open_tickets + citrix_open_tickets + microsoft365_open_tickets,
    snapshot_time
FROM `test_scenario_days`;

-- KPIs transversales normalizados.
...
SELECT
    ...
    CASE kpi.kpi_code
        ...
        WHEN 'operational_backlog' THEN LEAST(100, 18 + days.glpi_operational_pressure)
...
-- Snapshots agregados para el panel de analisis.
...
SELECT
    ...
    -- global_status es numerico en esta tabla.
    global_criticality,
