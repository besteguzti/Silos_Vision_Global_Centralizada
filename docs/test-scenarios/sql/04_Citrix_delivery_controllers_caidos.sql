-- Escenario 04: Citrix caido por Delivery Controllers no disponibles.
--
-- Objetivo:
-- - Simular 90 dias historicos.
-- - Mantener Aruba correcto.
-- - Mantener Microsoft 365 correcto y con usuarios activos.
-- - Degradar Citrix progresivamente hasta dejar todos los Delivery Controllers caidos.
-- - Mostrar sesiones Citrix a 0 en el ultimo periodo.
-- - Aumentar tickets GLPI relacionados con Citrix.
--
-- Recomendacion:
-- Ejecutar antes:
...
SELECT
    ...
    -- Aruba se mantiene correcto durante todo el escenario.
    ...
    -- Microsoft 365 se mantiene correcto.
    1200 + MOD(base.day_index, 50) AS microsoft365_active_users,
    8 + MOD(base.day_index, 3) AS microsoft365_open_tickets,

    -- Delivery Controllers Citrix: degradacion progresiva hasta 0.
    CASE
        WHEN base.day_index <= 49 THEN 4
        WHEN base.day_index <= 59 THEN 3
        WHEN base.day_index <= 64 THEN 2
        WHEN base.day_index <= 69 THEN 1
        ELSE 0
    END AS citrix_available_delivery_controllers,

    -- Sesiones Citrix: normales al inicio, bajan con la degradacion y acaban en 0.
    CASE
        WHEN base.day_index <= 49 THEN 180 + MOD(base.day_index, 12)
        WHEN base.day_index <= 69 THEN 170 - ((base.day_index - 50) * 6)
        ELSE 0
    END AS citrix_active_sessions,

    -- Errores de inicio Citrix: suben al caer los Delivery Controllers.
    CASE
        WHEN base.day_index <= 49 THEN MOD(base.day_index, 3)
        WHEN base.day_index <= 69 THEN 10 + ((base.day_index - 50) * 2) + MOD(base.day_index, 4)
        ELSE 55 + MOD(base.day_index, 10)
    END AS citrix_failed_logons,

    -- Tickets Citrix: suben claramente.
    CASE
        WHEN base.day_index <= 49 THEN 10 + MOD(base.day_index, 4)
        WHEN base.day_index <= 69 THEN 20 + ((base.day_index - 50) * 2) + MOD(base.day_index, 4)
        ELSE 75 + (base.day_index - 70) + MOD(base.day_index, 3)
    END AS citrix_open_tickets,

    -- Afeccion tecnica Citrix normalizada.
    CASE
        WHEN base.day_index <= 49 THEN 4 + MOD(base.day_index, 3)
        WHEN base.day_index <= 69 THEN 30 + (base.day_index - 50)
        ELSE 70 + (base.day_index - 70)
    END AS citrix_affectation,

    -- Presion operativa GLPI provocada por Citrix.
    CASE
        WHEN base.day_index <= 49 THEN 6 + MOD(base.day_index, 4)
        WHEN base.day_index <= 69 THEN 26 + (base.day_index - 50)
        ELSE 64 + (base.day_index - 70)
    END AS glpi_operational_pressure,

    -- Degradacion tecnica agregada.
    CASE
        WHEN base.day_index <= 49 THEN 5 + MOD(base.day_index, 3)
        WHEN base.day_index <= 69 THEN 32 + (base.day_index - 50)
        ELSE 66 + (base.day_index - 70)
    END AS technical_degradation,

    -- Impacto en usuarios por imposibilidad de acceder a Citrix.
    CASE
        WHEN base.day_index <= 49 THEN 3 + MOD(base.day_index, 3)
        WHEN base.day_index <= 69 THEN 30 + (base.day_index - 50)
        ELSE 68 + (base.day_index - 70)
    END AS user_impact,

    -- Criticidad global. El problema es importante, pero acotado a Citrix.
    CASE
        WHEN base.day_index <= 49 THEN 5 + MOD(base.day_index, 3)
        WHEN base.day_index <= 69 THEN 25 + (base.day_index - 50)
        ELSE 42 + (base.day_index - 70)
    END AS global_criticality,

    CASE
        WHEN base.day_index <= 49 THEN 0
        WHEN base.day_index <= 69 THEN 20
        ELSE 40
    END AS affected_services_percent,

    CASE
        WHEN base.day_index <= 49 THEN 0
        WHEN base.day_index <= 69 THEN 8 + MOD(base.day_index, 4)
...
-- Historico Citrix: Delivery Controllers caen y las sesiones llegan a 0.
...
SELECT
    ...
    4,
    CASE
        WHEN day_index <= 49 THEN MOD(day_index, 3)
        WHEN day_index <= 69 THEN 8 + MOD(day_index, 5)
        ELSE 18 + MOD(day_index, 8)
    END,
    CASE
        WHEN day_index <= 49 THEN 10 + MOD(day_index, 6)
        WHEN day_index <= 69 THEN 35 + MOD(day_index, 20)
        ELSE 75 + MOD(day_index, 15)
    END,
    CASE
        WHEN day_index <= 49 THEN 30 + MOD(day_index, 12)
        WHEN day_index <= 69 THEN 50 + MOD(day_index, 20)
        ELSE 0
    END,
...
-- Historico Microsoft 365: servicio sano y usuarios activos normales.
...
-- Historico GLPI: presion centrada en tickets Citrix.
...
SELECT
    ...
    CASE
        WHEN day_index <= 49 THEN 0
        WHEN day_index <= 69 THEN 2 + MOD(day_index, 3)
        ELSE 8 + MOD(day_index, 4)
    END,
    CASE
        WHEN day_index <= 49 THEN 0
        WHEN day_index <= 69 THEN 4 + MOD(day_index, 3)
        ELSE 12 + MOD(day_index, 5)
    END,
    CASE
        WHEN day_index <= 49 THEN 6
        WHEN day_index <= 69 THEN 12
        ELSE 20
    END,
    CASE
        WHEN day_index <= 49 THEN 20 + MOD(day_index, 4)
        WHEN day_index <= 69 THEN 32 + MOD(day_index, 6)
        ELSE 52 + MOD(day_index, 8)
    END,
    CASE
        WHEN day_index <= 49 THEN 20 + MOD(day_index, 4)
        WHEN day_index <= 69 THEN 18 + MOD(day_index, 4)
        ELSE 15 + MOD(day_index, 4)
    END,
    120 + MOD(day_index, 6) + aruba_open_tickets + citrix_open_tickets + microsoft365_open_tickets,
    CASE
        WHEN day_index <= 49 THEN 120 + MOD(day_index, 6)
        WHEN day_index <= 69 THEN 100 + MOD(day_index, 6)
        ELSE 75 + MOD(day_index, 6)
    END,
    aruba_open_tickets + citrix_open_tickets + microsoft365_open_tickets,
    snapshot_time
FROM `test_scenario_days`;
